...
echo "notify_master.sh - $(date): Master transitioned to MASTER state. Initiating failback procedure." | tee -a "$LOGFILE"
...
echo "notify_master.sh - $(date): SSHing to Manager ($MANAGER_IP) to start rsync and wait for completion." | tee -a "$LOGFILE"
if ! ssh "$MANAGER_USER@$MANAGER_IP" "bash -c '/usr/local/bin/manager_perform_rsync.sh && touch $SYNC_COMPLETE_FILE'" | tee -a "$LOGFILE"; then
    echo "notify_master.sh - $(date): ERROR: Failed to initiate or complete rsync on Manager." | tee -a "$LOGFILE"
    exit 1
fi

# --- Step 2: Master waits for Manager's rsync and then starts its own containers ---
echo "notify_master.sh - $(date): Rsync initiated on Manager. Waiting for $SYNC_COMPLETE_FILE on Manager..." | tee -a "$LOGFILE"

# Wait for the flag file to appear on the Manager
TIMEOUT=300 # 5 minutes timeout
ELAPSED=0
while [ ! -f "$SYNC_COMPLETE_FILE" ]; do
    echo "notify_master.sh - $(date): Waiting for $SYNC_COMPLETE_FILE on Manager... (Elapsed: ${ELAPSED}s)" | tee -a "$LOGFILE"
    sleep 5
    ELAPSED=$((ELAPSED+5))
    if [ $ELAPSED -ge $TIMEOUT ]; then
        echo "notify_master.sh - $(date): ERROR: Timeout waiting for rsync completion flag from Manager." | tee -a "$LOGFILE"
        exit 1
    fi
...
echo "notify_master.sh - $(date): Rsync complete flag detected on Manager. Starting local application services." | tee -a "$LOGFILE"
if ! "$MASTER_APP_RESTART_SCRIPT" | tee -a "$LOGFILE"; then
    echo "notify_master.sh - $(date): ERROR: Failed to restart applications on Master." | tee -a "$LOGFILE"
    exit 1
fi

# --- Step 3: Master tells Manager to release VIP ---
# After Master's apps are up, tell Manager to stop Keepalived and release VIP.
echo "notify_master.sh - $(date): Master apps restarted. Telling Manager ($MANAGER_IP) to release VIP." | tee -a "$LOGFILE"
if ! ssh "$MANAGER_USER@$MANAGER_IP" "/usr/local/bin/manager_release_vip.sh" | tee -a "$LOGFILE"; then
    echo "notify_master.sh - $(date): ERROR: Failed to instruct Manager to release VIP." | tee -a "$LOGFILE"
    exit 1
fi

echo "notify_master.sh - $(date): Failback orchestration complete. VIP should now be on Master." | tee -a "$LOGFILE"
...
echo "master_restart_apps.sh - $(date): Initiating application service restart on Master." | tee -a "$LOGFILE"

# --- Stop Docker Containers ---
echo "master_restart_apps.sh - $(date): Stopping specified Docker containers (if running)..." | tee -a "$LOGFILE"
for container_name in "${DOCKER_CONTAINERS[@]}"; do
    if docker ps -a --format '{{.Names}}' | grep -Eq "^${container_name}$"; then
        echo "master_restart_apps.sh - $(date): Stopping Docker container: ${container_name}" | tee -a "$LOGFILE"
        docker stop "$container_name" || {
            echo "master_restart_apps.sh - $(date): WARNING: Failed to stop container ${container_name}. Continuing..." | tee -a "$LOGFILE"
        }
    else
        echo "master_restart_apps.sh - $(date): Container ${container_name} not found or not running. Skipping stop." | tee -a "$LOGFILE"
    fi
done
...
echo "master_restart_apps.sh - $(date): Starting specified Docker containers..." | tee -a "$LOGFILE"
for container_name in "${DOCKER_CONTAINERS[@]}"; do
    echo "master_restart_apps.sh - $(date): Starting Docker container: ${container_name}" | tee -a "$LOGFILE"
    ...
    for i in $(seq 1 $HEALTH_CHECK_RETRIES); do
        if docker inspect -f '{{.State.Running}}' "$container_name" | grep -q "true"; then
            echo "master_restart_apps.sh - $(date): Container ${container_name} is running." | tee -a "$LOGFILE"
            CONTAINER_STARTED=true
            break
        fi
        echo "master_restart_apps.sh - $(date): Waiting for container ${container_name} to start... (${i}/${HEALTH_CHECK_RETRIES})" | tee -a "$LOGFILE"
        sleep "$HEALTH_CHECK_DELAY"
    done

    if [ "$CONTAINER_STARTED" = false ]; then
        echo "master_restart_apps.sh - $(date): ERROR: Docker container ${container_name} failed to start on Master after multiple attempts." | tee -a "$LOGFILE"
        exit 1 # Exit if any critical container fails to start
    fi
done

# --- Start Systemd Services ---
echo "master_restart_apps.sh - $(date): Starting specified Systemd services..." | tee -a "$LOGFILE"
for service_name in "${SYSTEMD_SERVICES[@]}"; do
    echo "master_restart_apps.sh - $(date): Starting Systemd service: ${service_name}" | tee -a "$LOGFILE"
    systemctl start "$service_name"
    if ! systemctl is-active --quiet "$service_name"; then
        echo "master_restart_apps.sh - $(date): ERROR: Systemd service ${service_name} failed to start on Master." | tee -a "$LOGFILE"
        exit 1 # Exit if any critical service fails to start
    fi
    echo "master_restart_apps.sh - $(date): Systemd service ${service_name} started successfully." | tee -a "$LOGFILE"
done

echo "master_restart_apps.sh - $(date): All specified application services processed successfully on Master." | tee -a "$LOGFILE"
